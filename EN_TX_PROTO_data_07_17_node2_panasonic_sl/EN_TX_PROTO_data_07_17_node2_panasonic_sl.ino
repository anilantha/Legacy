...
//int temperature = 21;      // Example temperature value
//int cap = 32;       // Example capacity value
//int nodeID = 2;     // Example node ID value
//
//// Concatenate the values into a 16-bit hexadecimal value
//int payload = ((temperature & 0xFF) << 12) | ((cap & 0xFF) << 4) | (nodeID & 0x0F);

int temperature = TEMP;  // Example temperature value
int cap = Sense_storage(); // Example capacity value
cap = cap * 10;
int nodeID = 1;          // Example node ID value
int PV = 28 + random(2) * 1;;             // Example PV value

// Concatenate the values into a 28-bit hexadecimal value
unsigned long payload = ((unsigned long)(temperature & 0xFF) << 24) | ((unsigned long)(cap & 0xFF) << 16) | ((unsigned long)(nodeID & 0xFF) << 8) | (PV & 0xFF);
...
//    irsend.sendRC5(TEMP, 8);
   irsend.sendPanasonic(0xABC0,payload);
