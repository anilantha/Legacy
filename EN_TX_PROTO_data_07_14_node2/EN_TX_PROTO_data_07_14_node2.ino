...
// Concatenate the values into a 20-bit hexadecimal value
unsigned long payload = ((unsigned long)(temperature & 0xFF) << 16) | ((unsigned long)(cap & 0xFF) << 8) | (nodeID & 0xFF);

  delay(2);
  for (int i = 0; i < 1; i++) {
    // irsend.sendPanasonic(0x4004, 0xABCDEF1);
//    irsend.sendRC5(TEMP, 8);
   irsend.sendRC5(payload, 24);
